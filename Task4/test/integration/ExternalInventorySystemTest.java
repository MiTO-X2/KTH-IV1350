...
class ExternalInventorySystemTest {
    ...
    @Test
    void testRetrieveItemInformation()  throws ItemNotFoundException, DatabaseFailureException {
    ...
    @Test
    void testUpdateInventory() throws ItemNotFoundException, DatabaseFailureException {
    ...
    @Test
    void testRetrieveItemInformation_ItemNotFound() {
        assertThrows(ItemNotFoundException.class, () -> {
            inventorySystem.retrieveItemInformation("999");
        }, "Should throw ItemNotFoundException for invalid identifier.");
    }

    @Test
    void testRetrieveItemInformation_DatabaseFailure() {
        assertThrows(DatabaseFailureException.class, () -> {
            inventorySystem.retrieveItemInformation("DBFAIL");
        }, "Should throw DatabaseFailureException for identifier '99'.");
    }

    @Test
    void testUpdateInventoryWithEmptyListDoesNotThrow() throws ItemNotFoundException, DatabaseFailureException {
        SaleDTO saleDTO = new SaleDTO(new ArrayList<>(), new Amount(0), new Amount(0));
        assertDoesNotThrow(() -> {
            inventorySystem.updateInventory(saleDTO);
        }, "Updating inventory with empty sold items list should not throw exception.");
    }

    @Test
    void testCheckIfItemIdentifierValidWithSpecialCharacters() {
        assertFalse(inventorySystem.checkIfItemIdentifierValid("!@#$%^&*()"), "Special characters identifier should be invalid.");
        assertFalse(inventorySystem.checkIfItemIdentifierValid(" "), "Whitespace identifier should be invalid.");
    }
}